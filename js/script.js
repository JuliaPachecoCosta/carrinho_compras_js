...
// Manipulação dos dados do modelo a partir do JSON (Usando forEach no lugar de map para melhor performance)
modelsJson.forEach((item, index) => {
    ...
    // Adicionar evento de clique ao exibir o modal ao clicar no modelo
...
// Adicionar eventos de clique para diminuir a quantidade do modal
...
// Adicionar eventos de clique para aumentar a quantidade do modal
...
function updateCart() {
    ...
    if (cart.length > 0) {
        ...
        cart.forEach((itemCart, index) => {
            ...
            // Atualizar os dados do item clonado do carrinho (O nome do tamanho é puxado direto do array sizes)
